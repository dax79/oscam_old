...
int32_t cs_open_logfiles(void)
{
	...
	cs_log(">> OSCam << cardserver %s, version " CS_VERSION ", build r" CS_SVN_VERSION " (" CS_TARGET ") Compiled by Dax", starttext);
								...
								uint8_t* hdr_date_offset, uint8_t* hdr_time_offset, uint8_t* hdr_info_offset)
{
	...
	tmp = snprintf(txt, txt_size, "[LOG000]%02d/%02d/%04d %02d:%02d:%02d %08X %c ",
		lt.tm_mday,
		lt.tm_mon + 1,
		lt.tm_year + 1900,
		lt.tm_hour,
