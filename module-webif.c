		...
		for(; rowcount < statsize; ++rowcount)
		{
			...
			if(!(s->rc == rc2hide) && ((rc2show == -1) || (s->rc == rc2show)))
			{
				...
				else
				{
					...
					{
						char tbuffer [30];
						strftime(tbuffer, 30, "%d-%m-%YT%H:%M:%S%z", &lt);
				...
				if(rdr->ll_entitlements)
				{
					...
					while((item = ll_iter_next(&itr)))
					{
						...
						if(!apicall)
							...
							{ strftime(tbuffer, 30, "%d-%m-%YT%H:%M:%S%z", &start_t); }
						tpl_addVar(vars, TPLADD, "ENTSTARTDATE", tbuffer);

						if(!apicall)
							{ strftime(tbuffer, 30, "%d-%m-%Y", &end_t); }
						else
							{ strftime(tbuffer, 30, "%d-%m-%YT%H:%M:%S%z", &end_t); }
...
static char *send_oscam_failban(struct templatevars * vars, struct uriparams * params, int8_t apicall)
{
	...
	while((v_ban_entry = ll_iter_next(&itr)))
	{
		...
		{
			char tbuffer [30];
			strftime(tbuffer, 30, "%d-%m-%YT%H:%M:%S%z", &st);
